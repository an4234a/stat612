...
map_1 <- map %>%
  ...
  left_join(ACS_income, by = "ward") %>%
  st_join(intersect, by = "ward")
##south area in DC had lower median income

ggplot(map_1) + geom_sf(aes(fill = value)) 
crime_income_pw <- map_1 %>%
...
ACS_unemployment <- ACS %>%
  filter(str_detect(desc,"Unemployment Rate"),
         key == "Percent")%>%
  mutate(value = str_replace_all(value,",",""),
         value = parse_number(value))
...
map_2 <- map %>%
  separate(NAME, into= c("ward1", "ward"), sep = " ") %>%
  left_join(ACS_unemployment, by = "ward") %>%
  st_join(intersect, by = "ward")


ggplot(map_2) + geom_sf(aes(fill = value)) 
crime_unemployment_pw <- map_2 %>%
  select(ward.x, value,n) %>%
  arrange(desc(value,n))
ggplot(crime_unemployment_pw, aes(value,n)) + geom_point()+ geom_smooth(method= lm, se=FALSE)
